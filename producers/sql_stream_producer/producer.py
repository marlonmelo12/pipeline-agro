# producers/sql_stream_producer/producer.py (VERSÃO COM AGRUPAMENTO POR DATA)
...
def run_sql_stream_producer():
    ...
    while True:
        try:
            # Lê todos os dados da tabela, ordenados por data
            df = pd.read_sql(f"SELECT * FROM {TABLE_NAME} ORDER BY data", engine)
            df['data'] = pd.to_datetime(df['data'])
            
            # --- MUDANÇA PRINCIPAL AQUI ---
            # Agrupa o DataFrame pela coluna 'data'
            grouped_by_date = df.groupby('data')

            # Itera sobre cada grupo (cada dia)
            for date, group_df in grouped_by_date:
                # Converte o grupo de linhas daquele dia para uma lista de dicionários
                ...
                # Envia a LISTA inteira como uma única mensagem
                producer.send(KAFKA_TOPIC, message_payload)
                producer.flush()
                # Um delay menor, pois estamos enviando um dia inteiro de uma vez
                time.sleep(10)
            # --- FIM DA MUDANÇA ---
